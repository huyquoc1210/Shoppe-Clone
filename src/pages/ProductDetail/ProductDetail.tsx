...
const ProductDetail = () => {
  ...
  const { name, images, price, price_before_discount, quantity, rating, description, sold, _id } =
    productDetail;

  const currentImages = images.slice(...indexImage) || [];
  ...
  const handlePrev = () => {
    if (indexImage[0] > 0) {
      setIndexImage((prev) => [prev[0] - 1, prev[1] - 1]);
    }
  };
  ...
  const handleZoom = (event: MouseEvent<HTMLDivElement>) => {
    ...
    // const offsetY = event.pageY - (rect.x + window.screenY);
  ...
  const createToCart = () => {
    addToCartMutation.mutate(
      { buy_count: buyCount, product_id: _id },
      {
        onSuccess: (data) => {
